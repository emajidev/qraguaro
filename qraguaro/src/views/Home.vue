<template>
  <div class="home">
    ...
    <div id="list"  v-for = "(item , index) in  users" >
      ...
      <ul>
<!--       <li>codigo Qr {{item.QR}} </li> - <li> {{item.name}} </li> - <li> {{item.lastName}} </li> - <li> {{item.cid}} </li>- <li> {{item.email}} </li> <li><button v-on:click="deliteUser(item.id , index)" >Eliminar</button></li>
 -->       
      <li>   </li><li></li> 
